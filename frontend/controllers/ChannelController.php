...
class ChannelController extends Controller
{
    ...
    public function actionSubscribe($username)
    {
        ...
        if (!$subscriber) {
            ...
            $subscriber->save();
            Yii::$app->mailer->compose(['html' => 'subscriber-html', 'text' => 'subscriber-text'], ['channel' => $channel, 'user' => Yii::$app->user->identity])
                ->setFrom([Yii::$app->params['senderEmail'] => 'YouTube Notifications'])
                ->setTo($channel->email)
//                ->setTo('mayank.singhal@samarth.ac.in')
                ->setSubject('New Subscription Alert from YouTube')
                ->send();
        } else {
            $subscriber->delete();
        }
